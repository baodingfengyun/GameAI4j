...
/**
 * 非线程安全的信号量<br>
 * A non-blocking semaphore that does not ensure the atomicity of its
 * operations, meaning that it's not tread-safe.
 *
 * @author davebaol
 */
public class SimpleNonBlockingSemaphore implements NonBlockingSemaphore {
    ...
    /**
     * Creates a {@code SimpleNonBlockingSemaphore} with the given name and number
     * of resources.
     *
    ...
    public boolean acquire(int resources) {
        if (acquiredResources + resources <= maxResources) {
            ...
            // System.out.println("sem." + name + ": acquired = TRUE, acquiredResources = "
            // + acquiredResources);
            return true;
        }
        // System.out.println("sem." + name + ": acquired = FALSE, acquiredResources = "
        // + acquiredResources);
        return false;
    }
    ...
    public boolean release(int resources) {
        if (acquiredResources - resources >= 0) {
            acquiredResources -= resources;
            // System.out.println("sem." + name + ": released = TRUE, acquiredResources = "
            // + acquiredResources);
            return true;
        }
        // System.out.println("sem." + name + ": released = FALSE, acquiredResources = "
        // + acquiredResources);
        return false;
    }
     ...
     * {@link SimpleNonBlockingSemaphore}.
     *
     * @author davebaol
     */
