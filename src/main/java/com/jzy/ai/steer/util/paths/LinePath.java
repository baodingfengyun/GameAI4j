 ...
 */
public class LinePath<T extends Vector<T>> implements Path<T, LinePath.LinePathParam> {
     ...
     */
    public LinePath(List<T> waypoints) {
     ...
     * @throws IllegalArgumentException if {@code waypoints} is {@code null} or has less than two (2) waypoints.
     */
    public LinePath(List<T> waypoints, boolean isOpen) {
        this.isOpen = isOpen;
        createPath(waypoints);
        nearestPointOnCurrentSegment = waypoints.get(0).cpy();
        nearestPointOnPath = waypoints.get(0).cpy();
        tmpB = waypoints.get(0).cpy();
        tmpC = waypoints.get(0).cpy();
    }
    ...
    public T getStartPoint() {
        return segments.get(0).begin;
    }

    @Override
    public T getEndPoint() {
        return segments.get(0).end;
    }
    ...
    public float calculatePointSegmentSquareDistance(T out, T a, T b, T c) {
        ...
        if (abLen2 != 0) {
            ...
            out.mulAdd(ab, MathUtil.clamp(t, 0, 1));
        }
    ...
    public void createPath(List<T> waypoints) {
        ...
        segments = new ArrayList<>(waypoints.size());
        pathLength = 0;
        T curr = waypoints.get(0);
        T prev = null;
        for (int i = 1; i <= waypoints.size(); i++) {
            ...
            if (i < waypoints.size())
                ...
                curr = waypoints.get(0); // close the path
